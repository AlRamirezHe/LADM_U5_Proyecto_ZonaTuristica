...
import kotlinx.android.synthetic.main.fragment_pantalla_bebidas.view.*
...
class PantallaBebidas(activity: MainActivity) : Fragment() {
    ...
    override fun onCreateView(
        ...
        val vista = inflater.inflate(R.layout.fragment_pantalla_bebidas, container, false)
        try {
            activity.negocio.listaBebidas.forEach {
                mostrarBebidas(vista, it)
            }
    ...
    fun mostrarBebidas( vista: View , itemMenu: ItemMenu) {
        // LINEAR LAYOUT PRINCIPAL
        val contenedorItem = LinearLayout(activity)
        vista.contenedorBebidas.addView(contenedorItem)
        ...
        // IMAGE VIEW BEBIDA
        //=================================================
        val imagenBebida = ImageView(activity)
        contenedorImg.addView(imagenBebida)

        val p3 = imagenBebida.layoutParams as FrameLayout.LayoutParams
        p3.width = FrameLayout.LayoutParams.MATCH_PARENT
        p3.height = FrameLayout.LayoutParams.MATCH_PARENT
        imagenBebida.layoutParams = p3

        if( itemMenu.imagen == null ) {
            imagenBebida.setImageResource( R.drawable.icono_bebida )
            imagenBebida.setBackgroundColor(ContextCompat.getColor( activity , R.color.rojoLigero))
            imagenBebida.setColorFilter(
                ContextCompat.getColor( activity , R.color.rojoOscuro),
                android.graphics.PorterDuff.Mode.SRC_IN
            )
        }
        else {
            imagenBebida.setImageBitmap( itemMenu.imagen )
            imagenBebida.scaleType = ImageView.ScaleType.CENTER_CROP
        }
